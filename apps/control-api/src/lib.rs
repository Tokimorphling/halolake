...
use storage::{
    DeleteUsageBeforeRequest, ListOptionsRequest, ManagementStore, OptionStore,
...
#[derive(Debug, Clone, Copy, Default, Deserialize)]
struct DeleteLogQuery {
    #[serde(default)]
    target_timestamp: i64,
}

#[derive(Debug, Clone, Copy, Default, Deserialize)]
struct LogCleanupTaskQuery {
    #[serde(default)]
    target_timestamp: i64,
}

#[derive(Debug, Clone, Default, Deserialize)]
struct CurrentSystemTaskQuery {
    #[serde(rename = "type", default)]
    task_type: String,
}

#[derive(Debug, Clone, Copy, Default, Deserialize)]
struct ListSystemTaskQuery {
    #[serde(default)]
    limit: usize,
}
...
#[derive(Debug, Clone, Default, Deserialize)]
struct ChannelAffinityUsageCacheQuery {
    #[serde(default)]
    rule_name: String,
    #[serde(default)]
    using_group: String,
...
async fn search_logs_deprecated() -> Response {
    Json(ApiResponse::<()>::error("该接口已废弃")).into_response()
}

async fn get_channel_affinity_usage_cache_stats(
    State(state): State<AppState>,
    headers: HeaderMap,
...
async fn create_log_cleanup_system_task(
    State(state): State<AppState>,
    headers: HeaderMap,
...
async fn get_current_system_task(
    State(state): State<AppState>,
    headers: HeaderMap,
...
async fn list_system_instances(State(state): State<AppState>, headers: HeaderMap) -> Response {
    let _actor = match require_role(&state, &headers, ROLE_ROOT_USER).await {
        Ok(user) => user,
        Err(resp) => return resp,
    };
...
async fn delete_stale_system_instances(
    State(state): State<AppState>,
    headers: HeaderMap,
) -> Response {
    let _actor = match require_role(&state, &headers, ROLE_ROOT_USER).await {
...
async fn delete_stale_system_instance(
    State(state): State<AppState>,
    headers: HeaderMap,
...
async fn web_fallback(State(state): State<AppState>, method: Method, uri: Uri) -> Response {
...
fn usage_log_stats(state: &AppState, query: &LogQuery, user_id: Option<&str>) -> Response {
...
fn usage_log_records_from_events(
    state: &AppState,
...
fn token_usage_ids(token: &TokenRecord) -> BTreeSet<String> {
...
fn usage_log_other(event: &UsageEvent) -> String {
...
fn usage_event_quota_value(event: &UsageEvent) -> i64 {
    event
        .quota
        .unwrap_or_else(|| event.observed_tokens().min(i64::MAX as u64) as i64)
}

struct UsageGroupLookup {
    user_groups: HashMap<String, String>,
    token_groups: HashMap<String, String>,
    channel_groups: HashMap<String, String>,
}
...
fn usage_log_type(status: UsageStatus) -> i32 {
    match status {
...
fn validate_flow_time_range(query: &DataQuery) -> Result<(), Response> {
