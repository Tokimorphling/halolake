...
- control-api 支持 management storage backend：
  ...
  - `examples/control-api-sqlite.toml` 提供可运行示例
...
- control-api 默认仍是 memory store；配置 SQLite 后 users/tokens/channels/
  model_mappings、usage_events、options、catalog、redemptions/topups 会持久化。
- 用户密码已按 bcrypt hash 存入管理数据；SQLite schema/migration 已有，
  但 Postgres/MySQL 和完整 new-api schema 迁移尚未实现。配置层已能识别 new-api
  风格 `SQL_DSN`/`LOG_SQL_DSN`，真正的跨数据库 store 仍待实现。
...
- 已发现 model status_only 请求在 HTTP 层 422。
- 默认 memory + SQLite 可用，Postgres/MySQL 与生产级 session 仍未完成。
