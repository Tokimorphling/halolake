...
| `OTEL_EXPORTER_OTLP_ENDPOINT` | 未设置 | 设置后导出 OTLP。HTTP 填 **base**（本地 `http://…:4318` 或 Grafana `https://otlp-gateway-….grafana.net/otlp`）；SDK 会再拼 `/v1/traces|logs|metrics`。不要把完整 signal URL 写进 `with_endpoint` 语义里 |
